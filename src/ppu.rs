...
//! tuiles $8800-$97FF) selon Pan Docs « Background »/« Window ».
...
/// Bit 5 du LCDC : activation de la fenêtre (carte $9C00-$9BFF, tuiles $8800-$97FF).
...
impl PPU {
    ...
    /// Rend la frame complète dans le framebuffer depuis les registres courants et la VRAM
    /// (Pan Docs « Background »/« Window ») : d'abord la couche Background défilée par SCX/SCY,
    /// puis la fenêtre si activée. Le LCD éteint (bit 7 du LCDC à 0) rend l'écran noir.
    pub fn render_frame(&mut self, vram: &[u8; 0x2000]) {
        ...
        for y in 0..SCREEN_HEIGHT as u32 {
            ...
            for x in 0..SCREEN_WIDTH as u32 {
                ...
                self.framebuffer[y as usize * SCREEN_WIDTH + x as usize] = Self::shade(self.bgp >> (pixel * 2));
            }

            // Fenêtre : pas de défilement ; elle apparaît à partir de la ligne WY et de la colonne WX+1 (Pan Docs « Window »).
            if self.lcdc & LCDC_WINDOW != 0 && y >= self.wy as u32 {
                ...
                for x in (self.wx as u32 + 1)..SCREEN_WIDTH as u32 {
                    ...
                    let pixel = Self::tile_pixel(
                        vram,
                        (TILE_SET_8800 as i32 + (tile_index as i8) as i32 * 16) as usize, // tuiles $8800-$97FF (indices signés)
                        win_y & 7,
                        win_x & 7,
                    );
                    if pixel != 0 {
                        // Les pixels de valeur 0 sont transparents : le fond passe au travers.
                        let index = y as usize * SCREEN_WIDTH + x as usize;
                        self.framebuffer[index] = Self::shade(self.bgp >> (pixel * 2));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn render_window_layer() {
        let mut ppu = PPU::new();
        ppu.write_register(0xFF40, 0xB1); // LCD allumé, fond (tuiles $8000-$8FFF) + fenêtre activés
        ppu.bgp = 0xE4; // teinte v pour une valeur de pixel v
